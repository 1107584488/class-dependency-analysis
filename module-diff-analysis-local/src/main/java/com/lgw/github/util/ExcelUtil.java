...
public class ExcelUtil {
     ...
     */
    public static void writeExcelForClass(String outFilePath, String outFileName) {
        ...
        // 写入二期master独有类工作表
        writeExcelForSheet(workbook, outFilePath, outFileName, "二期master独有类", Constants.ONLY_MASTER_CLASS_PATH_MAP);
//        writeExcelForMasterSheet(workbook, outFilePath, outFileName);

        // 写入当前域分支独有类工作表
//        writeExcelForTargetSheet(workbook, outFilePath, outFileName);
        writeExcelForSheet(workbook, outFilePath, outFileName, "当前域分支独有类", Constants.ONLY_TARGET_CLASS_PATH_MAP);

        // 写入二期master与当前域分支gap类工作表
//        writeExcelForGapSheet(workbook, outFilePath, outFileName);
        writeExcelForSheet(workbook, outFilePath, outFileName, "二期与当前域分支差异类", Constants.MASTER_TARGET_CLASS_DIFF_MAP);
    }

    public static void writeExcelForSheet(Workbook workbook, String outFilePath, String outFileName, String sheetName, Map<String, String> map) {
        // 判断工作表是否存在，存在则续写，不存在则新建并写标题行
        Sheet sheet = FileUtil.getSheet(workbook, sheetName);
        if (sheet == null) {
        ...
        try (FileOutputStream fileOut = new FileOutputStream(outFilePath + File.separator + outFileName + ".xlsx")) {
            ...
            for (Map.Entry<String, String> entry : map.entrySet()) {
                ...
                Row moduleRow = sheet.getRow(rowNum);
                if (moduleRow == null) {
                    moduleRow = sheet.createRow(rowNum);
                }
                moduleRow.createCell(0).setCellValue(className);
                moduleRow.createCell(1).setCellValue(classPathName);
                rowNum ++;
            }
    ...
    public static void writeExcelForMasterSheet(Workbook workbook, String outFilePath, String outFileName) {
        // 判断工作表是否存在，存在则续写，不存在则新建并写标题行
        Sheet masterClassSheet = FileUtil.getSheet(workbook, "二期master独有类");
        if (masterClassSheet == null) {
            System.out.println("sheet表创建或打开失败，sheet表为空");
            return;
        }
        if (Constants.ONLY_MASTER_CLASS_PATH_MAP.isEmpty()) {
            return;
        }
        // 开始画excel并填充数据
        try (FileOutputStream fileOut = new FileOutputStream(outFilePath + File.separator + outFileName + ".xlsx")) {
            int rowNum = 1;
            for (Map.Entry<String, String> entry : Constants.ONLY_MASTER_CLASS_PATH_MAP.entrySet()) {
                String className = entry.getKey();
                String classPathName = entry.getValue();
                // 切记！切记！切记！此处有大坑！直接sheet.creativeRow会覆盖当前行，其他列数据被清空！
                Row moduleRow = masterClassSheet.getRow(rowNum);
                if (moduleRow == null) {
                    moduleRow = masterClassSheet.createRow(rowNum);
                }
                moduleRow.createCell(0).setCellValue(className);
                moduleRow.createCell(1).setCellValue(classPathName);
                rowNum ++;
    ...
    public static void writeExcelForTargetSheet(Workbook workbook, String outFilePath, String outFileName) {
        // 判断工作表是否存在，存在则续写，不存在则新建并写标题行
        Sheet targetClassSheet = FileUtil.getSheet(workbook, "当前域分支独有类");
        if (targetClassSheet == null) {
            System.out.println("sheet表创建或打开失败，sheet表为空");
            return;
        }
        if (Constants.ONLY_TARGET_CLASS_PATH_MAP.isEmpty()) {
            return;
        }
        // 开始画excel并填充数据
        try (FileOutputStream fileOut = new FileOutputStream(outFilePath + File.separator + outFileName + ".xlsx")) {
            int rowNum = 1;
            for (Map.Entry<String, String> entry : Constants.ONLY_TARGET_CLASS_PATH_MAP.entrySet()) {
                String className = entry.getKey();
                String classPathName = entry.getValue();
                // 切记！切记！切记！此处有大坑！直接sheet.creativeRow会覆盖当前行，其他列数据被清空！
                Row moduleRow = targetClassSheet.getRow(rowNum);
                if (moduleRow == null) {
                    moduleRow = targetClassSheet.createRow(rowNum);
                }
                moduleRow.createCell(0).setCellValue(className);
                moduleRow.createCell(1).setCellValue(classPathName);
                rowNum ++;
    ...
    public static void writeExcelForGapSheet(Workbook workbook, String outFilePath, String outFileName) {
        // 判断工作表是否存在，存在则续写，不存在则新建并写标题行
        Sheet gapClassSheet = FileUtil.getSheet(workbook, "二期与当前域分支差异类");
        if (gapClassSheet == null) {
            System.out.println("sheet表创建或打开失败，sheet表为空");
            return;
        }
        ...
        try (FileOutputStream fileOut = new FileOutputStream(outFilePath + File.separator + outFileName + ".xlsx")) {
            ...
            for (Map.Entry<String, String> entry : Constants.MASTER_TARGET_CLASS_DIFF_MAP.entrySet()) {
                ...
                String classPathName = entry.getValue();
                // 切记！切记！切记！此处有大坑！直接sheet.creativeRow会覆盖当前行，其他列数据被清空！
                Row moduleRow = gapClassSheet.getRow(rowNum);
                if (moduleRow == null) {
                    moduleRow = gapClassSheet.createRow(rowNum);
                }
                moduleRow.createCell(0).setCellValue(className);
                moduleRow.createCell(1).setCellValue(classPathName);
                rowNum ++;
